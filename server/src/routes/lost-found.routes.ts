...
import { uploadSingle, validateFileSignature } from '../middleware/upload';
  ...
  async (req, res, next) => {
    try {
      ...
      if (!description || !category || !busNumber || !route || !foundDate) {
        return res.status(400).json({ status: 'error', code: 'VALIDATION_ERROR', message: 'Missing required fields' });
      }

      const photoBuffer = req.file?.buffer;
      const photoMimetype = req.file?.mimetype;
      // Validate file content against magic bytes
      if (photoBuffer && photoMimetype && !validateFileSignature(photoMimetype, photoBuffer)) {
        return res.status(400).json({ status: 'error', code: 'VALIDATION_ERROR', message: 'Uploaded file does not appear to be a valid image' });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (action !== 'approve' && action !== 'reject') {
        return res.status(400).json({ status: 'error', code: 'VALIDATION_ERROR', message: 'Action must be approve or reject' });
      }
